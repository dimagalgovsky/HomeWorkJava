...
public class Задание {
    /*
    ручной режим - около 4часов
    Ханойские башни. Задача на умение работать с: Циклами, Массивами,
                        Методами, Консолью, Условиями, Файлами*:
-1. !!!ВНИМАНИЕ!!! не пытайтесь делать сразу автоматический режим игры, начинайте именно с ручного.
...
+3. На старте пользователю предлагают выбрать режим игры.
        С клавиатуры он выбирает режим. 1 - ручной, 2 - автоматический
+4. Дать возможность указать количество колец с которыми мы будет играть.
        Минимальное - 3, максимальное - 8
...
+6. Игровое поле в консоли отображает немного по другому.
        ...
        Пример: Если пользователь выберет что он будет играть с 4-мя кольцами то стартовое поле
         в консоли будет выглядеть следующим образом:
	1 * *
	2 * *
	3 * *
	4 * *
...
+8. После выбора пользователя вы должны проверить можно ли переместить выбранное кольцо
        на выбранный стержень.
+9. Если переместить можно то в консоли должно отобраться новое
        поле и предложение о выборе следующего кода.
 Пример:
Было:
	1 * *
	2 * *
	3 * *
	4 * *
Пользователь выбрал ход 1->3. Значит требуется переместить верхнее кольцо на первом стержне
        на третий стержень. Такой ход возможен - перемещаем:
	* * *
	2 * *
	3 * *
	4 * 1
+10. Если переместить нельзя то необходимо выдать сообщение о невозможности хода и снова
        предложить выбрать ход. Пример:
Было:
	* * *
	2 * *
	3 * *
	4 * 1
Пользователь выбрал ход 1->3. Значит требуется переместить верхнее кольцо на первом
        стержне на третий стержень. Такой ход не возможен, так как кольцо большего размера
        не может быть помещено на стержень с кольцом меньшего размера (кольцо 2 на стержень 3 с кольцом 1).
        Выводим сообщение: К сожалению данный ход не возможен правилами игры, выберите другой ход.
+11. При успешном завершении игры необходимо выводить поздравление и выводить количество
        ходов которые совершил пользователь\компьютер
...
13.* В автоматическом режиме после каждого хода компьютера должно выводиться
        получившееся поле (как при игре в ручном режиме)
