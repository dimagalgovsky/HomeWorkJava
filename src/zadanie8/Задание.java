...
Вам необходимо скачать https://www.dropbox.com/s/3b8posn819795i8/siteloader.zip?dl=0 данный архив содержит
        лпримеры кода которые вам необходимо использовать в своей работе.
        В классе SiteLoader реализовано подкючение по переданному адресу,
        в случае проблем с соединением (эксепшеном) происходит попытка повторного подключения.
        Для реализации пунктов без * данные классы можно использовать как есть,
        в случае выполнения задач с * вы можете изменять, переписывать и писать собственные классы вместо моих.
2. В классе NBRBLoader требуется дописать метод handle. При помощи данного метода
...
7**. Придумайте механизм защиты чтобы в файл нельзя было сохранить данные полученные от разных банков,
        в случае если пользователь указал адрес файла с курсами от другого банка необходимо
        дать возможность выбрать другой путь (5 раз). В случае превышения количества раз
        выдать ошибку и завершить работу программы с указанием спецефического кода выхода
     */
}
