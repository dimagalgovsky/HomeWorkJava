...
public class Задание {
    /*
    Коллекции, компараторы, полиморфизм:                                    //2 компоратора_по одному параметру и 2
...
+2. Создать компараторы для классов которые сравнивают:
	...
	+2.4. Возраст животных и клички животных по алфавиту
	        (Смотрите уточнение в пункте 2.99)
	+2.99. Особенность данных сортировок в том что у вас должна получиться сортировка
	        по двум полям, сначала по первому, потом по второму.
		...
		После сортировки должно получиться: [{{age: 1, nick: "Люцифер"},
		    age: 10, nick: "Анатолий"}, {age: 10, "Игнат"}, {age: 10, "Ярик"}].
		Так как сначала сортируется по возрасту в порядке возрастания,
		    а потом в рамках группу (age = 10), сортируется по кличке по возрастанию (по алфавиту).
+3. Работа с коллекциями. Выполняем действия над экземплярами классов созданных
        на основания задания 1:
	3.0* В идеале можно, используя полимормизм, написать метод под каждый
	    созданный в первом пункте класс + метод который будет выполнять основную работу над любыми коллекциями.
	        Например метод который генерирует Person и заполняет ими указанную вами коллекцию указанным
	        количеством  Person. Получившуюся коллекцию передаём во второй метод который выполняет все
	        остальные прописанные действия.
	+3.1. При помощи рандома генерируем 1_000_000
	        (Если компьютер глючит сделать 100_000) разных объектов одного класса.
...
-		3.3.2.* Сортируем собственным методом сортировки
	+3.4. Замерить время и распечатать консоль:
	            "Операция: ХХХХ. Заняла YYYY мс".
